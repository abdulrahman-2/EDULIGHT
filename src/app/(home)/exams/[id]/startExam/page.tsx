...
export default function Page() {
  ...
  if (!exam) return router.push('/not-found');
  ...
  return (
    <div className="py-16 max-w-5xl mx-auto">
      ...
      <div key={currentQuestion.id} className="mb-6">
        <p className="font-bold text-[42px] mb-6">
          {currentQuestion.question}
        </p>
        {currentQuestion.options.map((opt, i) => (
          <label key={i} className="block cursor-pointer relative mb-6 select-none">
            <input
              type="radio"
            ...
            />
 <div
    className={`flex items-center justify-between w-2/3 p-6 rounded-lg shadow-lg border-[#D4D1E7] border border-b-4 border-r-4 duration-300
      ${
  ...
  >              <span className="text-gray-800 text-[16px]">{opt}</span>
              <span className={`p-1.5 px-3 border text-[14px] border-[#D4D1E7] text-[#D4D1E7] rounded-lg  ${
        answers[currentQuestion.id] === opt
